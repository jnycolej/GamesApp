...
const reactionCooldownByPlayer =
  new Map();
  ...
  socket.on(
    ...
    (payload = {}, ack) => {
      try {
        ...
        const reaction = {
          id:
            `${code}-${socket.id}-${now}-${Math.random()
              .toString(16)
              .slice(2)}`,
